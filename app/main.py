...
from api import load_new_kbtopics_api, load_custom_topics_api, status, summarize_and_send_to_group_api, webhook, setup_api
import models  # noqa
from config import Settings
from whatsapp import WhatsAppClient
from whatsapp.init_groups import gather_groups
from utils.bedrock_embed_text import BedrockEmbeddingClient
...
async def lifespan(app: FastAPI):
    ...
    app.state.embedding_client = BedrockEmbeddingClient(region_name=settings.aws_region)
    try:
        yield
    finally:
...
app.include_router(load_custom_topics_api.router)
app.include_router(setup_api.router, tags=["Setup"])
