...
from utils.bedrock_embed_text import bedrock_embed_text
...
class KnowledgeBaseAnswers(BaseHandler):
    async def __call__(self, message: Message):
        ...
        # Get query embedding
        embedded_question = (
            await bedrock_embed_text(self.embedding_client, [rephrased_result.output])
        )[0]
    ...
    async def rephrasing_agent(
        ...
        rephrased_agent = Agent(
            ...
            - Your name is גוטמן (Goodman), your phone number is @{my_jid}
            - Attached is the recent chat history. You can use it to understand the context of the query. If the context is not clear or irrelevant to the query, ignore it.
            - ONLY answer with the new phrased query in English, no other text!""",
        )
